    ...
    for dia, letra in zip(dias, letras):
        ...
        for hora in horas:
            ...
            if columna in df.columns:
                clases = df[df[columna].notnull()][[columna, 'NOMBRE_COMPETENCIA', 'FICHA']].rename(columns={columna: 'AMB'}).to_dict('records')
                horario_dia[hora] = clases
        horario_instructor["horario"][dia] = horario_dia    

    return horario_instructor

def get_horario_instructor(name_instructor):    
...
def get_fichas_titular(name_instructor):    
    with open("static/horarios/1-2024.json","r", encoding='utf-8') as archivo_json:
        horario = json.load(archivo_json)
...
def get_sum_horas(name_instructor):
    horario_instructor = get_horario_instructor(name_instructor)
    sum_horas = 0
    for item in horario_instructor:
        sum_horas += item['horas_semanal']
    return sum_horas

def get_cant_fichas(name_instructor):
    horario_instructor = get_horario_instructor(name_instructor)
